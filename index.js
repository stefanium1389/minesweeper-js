    ...
    for (let i = 0; i < boardHeight; i++) {
        ...
        for (let j = 0; j < boardWidth; j++) {
            ...
            cell.addEventListener("mouseup", e => {
                switch (e.button) {
                    case 0: { mouseButtonPressed = false; onClick(i, j); unhighlightAdjecent(i, j); break; }
...
function onClick(i, j) {
    ...
    if(firstClicked && !minesPopulated)
    {
        populateMines(i,j)
    }
    if (board[i][j].state === "flag") {
